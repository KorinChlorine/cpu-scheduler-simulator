...
public class AlgorithmRegistry {
    ...
    public enum AlgoId {
        FCFS, SJF, SRT, RR, PRIORITY, PRIORITY_PREEMPTIVE, PRIORITY_RR, COMPARE_ALL
    }
    ...
    public static final List<AlgorithmMeta> ALL = Arrays.asList(
        ...
        new AlgorithmMeta(AlgoId.PRIORITY_PREEMPTIVE,  "6. Priority Scheduling (Preemptive)",         false, true),
        new AlgorithmMeta(AlgoId.PRIORITY_RR,          "7. Priority Scheduling + Round Robin",         true,  true),
        new AlgorithmMeta(AlgoId.COMPARE_ALL,          "8. Compare All Algorithms",                    true,  true)
    );
    ...
    public static SchedulingResult run(AlgoId id, List<Process> processes, int quantum) {
        return switch (id) {
            ...
            case PRIORITY_PREEMPTIVE -> Scheduler.priorityPreemptive(processes);
